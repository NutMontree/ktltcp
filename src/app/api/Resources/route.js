...
export async function POST(req) {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      const body = await req.json();
      resourceData = body.formData;
    } else if (contentType.includes("multipart/form-data")) {
      const formData = await req.formData();

      resourceData.year = formData.get("year");
      resourceData.department = formData.get("department");
      resourceData.namework = formData.get("namework");
      resourceData.nameproject = formData.get("nameproject");
      ...
      if (file && file.name) {
        ...
        resourceData.fileName = originalFilename;
        resourceData.fileUrl = `${originalFilename}`;
        resourceData.fileData = buffer; // <— เก็บไฟล์จริงในฐานข้อมูล
    ...
    return NextResponse.json({ message: "Create Resource" }, { status: 201 });
  } catch (err) {
